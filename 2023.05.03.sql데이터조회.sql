...
-- 약어 사용 시 AS : 생략 가능하지만, SELECT 절이 복잡해지면 가독성 측면에서 AS를 추가하는 개발자가 많다.
-- 라는 통계가 있음!!

SELECT 2*4 AS 곱, 2-1 AS 차, 2*3 AS 곱, 2/3 AS 몫
FROM    dual;

-- 2) WHERE 조건절
SELECT employee_id, last_name, salary, salary*12 AS 연봉, department_id
FROM    employees
--WHERE department_id = 80; -- 80번 부서에 근무하는 사원의 정보 조회, 34rows
...
[예제2-5] 한 해 동안 받은 급여가 120000($, USD)인 한 사원의 정보(어떤걸?)를 조회한다.
-- NLS 설정상 : 자동으로 오라클이 WON으로 설정되어 있음.
-- 다만, hr 스키마의 데이터는 영문 데이터로 자연스럽게 USD로 추측할 수 있음
SELECT employee_id, last_name, first_name, salary*12||''||'$' AS "연봉", department_id
FROM    employees
WHERE   salary * 12 = 120000;   -- 연봉 정보가 저장된 칼럼은 없지만, salary 컬럼에 12를 곱해서 연봉정보를 조회, 비교
-- 문자열 연결 연산자 : ||
-- 문자열 연결 함수 : CONCAT()
-- 문자 데이터, 숫자 데이터 : '로 묶음
...
[예제 2-8] 사번이 101번인 사원의 성과 한 해 동안 받은 급여를 조회한다.
-- 성 : name이라는 별칭
-- 한 해 동안 받은 급여 : ANNUAL SALARY라는 별칭

SELECT  employee_id AS 사번, last_name AS 성, salary * 12 AS "ANNUAL SALARY"
FROM    employees
WHERE   employee_id = 101;
...
FROM    employees
WHERE   salary <=3000;  -- 26 rows, 30번 50번 부서에 소속
...
SELECT  *
FROM    employees
WHERE   department_id > 80; -- 90번부터 나머지 부서들 ~11rows
...
SELECT  'hanul' AS employee_id, last_name, hire_date
FROM    employees
...
-- 부서정보는 DEPARTMENT에 있으나, 사원들은 부서 10~110번까지 소속되어 있음
...
[예제 2-14] 30번 부서 사원 중 급여가 10000 이하이면서 입사일자가 2005년 이전에 입사한 사원의 정보를 조회한다.
...
-- NLS : National Language Suppport (동아시아-한국/중국/일본) RR/MM/DD
...
[예제 2-18] 사번이 110번부터 120번까지의 사원 정보를 조회하시오.
-- 100번부터 206번 까지 총 107명 근무하는 회사의 사원정보 테이블
SELECT  *
FROM    employees
WHERE   employee_id >= 110
OR      employee_id <= 120; -- 107rows : 110 이상 + 120이하(=전체 사원 수)
...
SELECT  *
FROM    employees
WHERE   employee_id BETWEEN 110 AND 120; -- 11rows

-- (p.10 하단) NOT 연산자와 BETWEEN 연산자를 사용하여 BETWEEN 초기값 AND 마지막값과 반대되는 조건을 만든다.
...
SELECT  *
FROM    employees
WHERE   NOT employee_id BETWEEN 110 AND 120; -- 11rows
...
-- 11g 버전에서는 NLS 설정이 없어서 문제가 안 되지만, 21c에서는 RR/MM/DD형식을 따라서 조회하여야
-- 오류가 발생하지 않는다.
-- ORA-01858 : 숫자가 있어야 하는 위치에서 숫자가 아닌 문자가 발견되었습니다.라는 오류는 무조건 날짜형식이
-- 맞지 않아서 ==> 1) 대게는 변환함수 사용해서 처리하거나 
-- 2) NLS세팅을 확인하고 RR/MM/DD로 조회하거나
-- 도구 > 환경설정 > 데이터베이스 > NLS확인
SELECT  *
...
[예제 2-25] 30번 부서원 또는 60번 부서원 또는 90번 부서원의 정보를 조회한다.
...
FROM employees;
-- 178 Kimberely Grant (부서코드 NULL) : 프리랜서? 부서배정이 안 된? (=신입사원)
...
[예제 2-28] 이름이 K로 시작되는 사원 정보를 조회한다.
...
[예제 2-29] 성이 s로 끝나는 사원정보를 조회한다.
SELECT  *
FROM    employees
WHERE   last_name LIKE '%s';

[예제 2-30] 이름에 b가 들어가 있는 사원정보를 조회한다.
SELECT  *
FROM    employees
...
-- NUMBER(길이) : 정수
-- NUMBER(총길이,소숫점이하길이) : 실수
-- DATE : 날짜 데이터
-- ※ INT 자료형으로 컬럼을 정의하면 ==> 오라클 내부적으로 NUMBER로 변경해버림..
-- STRING 자료형으로 컬럼을 정의하면 ==>        "         VARCHAR2로    "
-- 빅데이터, 바이너리(=이미지, 오디오, 영상) ==> BLOB, CLOB, BFILE타입 ==> 쓰는 걸 본 적이 없음

[예제 2-31] 이메일의 세 번째 문자가 B인 사원정보를 조회한다.
SELECT  *
FROM    employees
WHERE   email LIKE '__B%';

[예제 2-32] 이메일이 뒤에서부터 세 번째 문자가 B인 사원정보를 조회한다.
SELECT  *
FROM    employees
...
[예제 2-33] 전화번호가 6으로 시작되지 않는 사원의 정보를 조회한다.
-- phone_number 컬럼 : 
...
WHERE   phone_number NOT LIKE '6%'; -- 61 rows (6으로 시작하지 않는)

-- ===========================================
-- Q. %나 _자체를 문자열로 조회하고자 한다면?
-- ===========================================
...
-- ESCAPE 식별자를 사용해 %나 _자체를 하나의 문자로서 검색할 수 있다.
...
SELECT  *
FROM    employees
WHERE   commission_pct IS NULL; -- 거래 수수료를 받지 않는 사람?? 72rows... 총 107명 중 35명은 받는 사람
